...
{
    log_info(">>Starting measurement<<");//开启运行调试
...
//        qDebug()<<">>close Measurement<<";//关闭调试口
...
extern int hpmicro_can ;

QString Backend::getPortName(CanInterfaceId id)
{
...
QString Backend::getInterfaceName(CanInterfaceId id)
{
    ...
    if (intf) {
        if(hpmicro_can)
        return intf->getDescription();//Colin
